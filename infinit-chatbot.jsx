...
const STARTER_QUESTIONS = [
  "Why is the sky blue?", "How do volcanoes erupt?", "What are black holes?",
  "How does the brain work?", "Why do we need sleep?", "How do plants make food?",
];
...
const formatMessage = (text) => {
  ...
  const isWebSearch = sourceType === "Web Search";
...
const GLOBAL_STYLES = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .auth-input{width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(88,135,255,0.15);border-radius:10px;padding:11px 14px;color:white;font-size:14px;font-family:'Nunito',sans-serif;font-weight:600;outline:none;transition:border-color 0.2s;}
  .auth-input:focus{border-color:rgba(88,135,255,0.4);}
...
const AuthScreen = ({ onAuth }) => {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: `radial-gradient(ellipse at 30% 20%,rgba(113,90,255,0.12) 0%,transparent 55%),radial-gradient(ellipse at 70% 80%,rgba(85,193,255,0.08) 0%,transparent 55%),${COLORS.dark}`, position: "relative", overflow: "hidden" }}>
      <AmbientField />
      <BackgroundLightField />
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 380, padding: "0 24px", animation: "fadeUp 0.4s ease-out" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <InfinitLogo size={48} />
          <div style={{ fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: 24, background: `linear-gradient(135deg,${COLORS.purple},${COLORS.sky})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginTop: 8 }}>infinit</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", fontWeight: 600, marginTop: 4 }}>science explorer</div>
        </div>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(88,135,255,0.12)", borderRadius: 16, padding: "28px 24px" }}>
          <div style={{ display: "flex", gap: 4, marginBottom: 24, background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 3 }}>
            {["signin", "signup"].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }} style={{ flex: 1, padding: "7px", borderRadius: 8, border: "none", background: mode === m ? "rgba(88,135,255,0.2)" : "transparent", color: mode === m ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.35)", fontSize: 12, fontFamily: "'Nunito',sans-serif", fontWeight: 700, transition: "all 0.15s" }}>
                {m === "signin" ? "Sign In" : "Sign Up"}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <input className="auth-input" type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} placeholder="Email" />
          ...
          {error && <div style={{ marginTop: 12, fontSize: 12, color: "#fca5a5", fontWeight: 600 }}>{error}</div>}
          {success && <div style={{ marginTop: 12, fontSize: 12, color: "rgba(85,193,255,0.9)", fontWeight: 600 }}>{success}</div>}
          <button onClick={submit} disabled={loading || !email || !password}
            style={{ width: "100%", marginTop: 16, padding: "12px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${COLORS.violet},${COLORS.blue})`, color: "white", fontSize: 14, fontFamily: "'Nunito',sans-serif", fontWeight: 700, opacity: loading || !email || !password ? 0.5 : 1, transition: "opacity 0.2s,transform 0.15s" }}
...
export default function App() {
  ...
  const [chatMode, setChatMode] = useState("normal"); // normal | quiz | socratic

  const messagesEndRef = useRef(null), inputRef = useRef(null);
  const fileInputRef = useRef(null), historyRef = useRef(null), userMenuRef = useRef(null);
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("conversations").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(30)
  ...
  const saveSession = async (msgs) => {
    ...
    const session = { user_id: user.id, title: msgs.find(m => m.role === "user")?.content?.slice(0, 40) || "Chat", messages: msgs, model: selectedModel };
    const { data } = await supabase.from("conversations").insert(session).select().single();
  ...
  const sendMessage = async (text) => {
    ...
    // cancel any previous controller just in case
    ...
    try {
      const res = await fetch(`${API_BASE}/chat/stream`, {
        ...
        body: JSON.stringify({ question, model: selectedModel, file_content: fileContent, mode: chatMode })
      });
      ...
      setMessages(prev => { const u = [...prev]; u[u.length - 1] = { role: "assistant", content: full, model: selectedModel, elapsed, mode: chatMode }; saveSession(u); return u; });
    } catch (err) {
      if (err.name === "AbortError") {
        // user stopped the response; finalize the last assistant message without error
        setMessages(prev => {
          const u = [...prev];
          const last = u[u.length - 1];
          if (last?.role === "assistant") {
            u[u.length - 1] = { ...last, streaming: false, aborted: true };
          }
          return u;
        });
      } else {
  ...
  const clearChat = () => { setMessages([WELCOME_MSG]); setShowWelcome(true); setLastQuestion(null); setChatMode("normal"); };
  const loadSession = (session) => { setMessages(session.messages); setSelectedModel(session.model); setShowWelcome(false); setShowHistory(false); };
  const deleteSession = async (id, e) => { e.stopPropagation(); await supabase.from("conversations").delete().eq("id", id); setHistory(prev => prev.filter(s => s.id !== id)); };
  const signOut = async () => { await supabase.auth.signOut(); setUser(null); setHistory([]); clearChat(); setShowUserMenu(false); };
  ...
  if (authLoading) return (
    <>
      ...
      <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: COLORS.dark }}>
        <div style={{ width: 24, height: 24, borderRadius: "50%", border: "2px solid rgba(88,135,255,0.2)", borderTop: `2px solid ${COLORS.sky}`, animation: "spin 0.8s linear infinite" }} />
      </div>
    </>
  );
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", position: "relative", overflow: "hidden", background: `radial-gradient(ellipse at 15% 0%,rgba(113,90,255,0.1) 0%,transparent 50%),radial-gradient(ellipse at 85% 100%,rgba(85,193,255,0.06) 0%,transparent 50%),${COLORS.dark}` }}>
        ...
        {showHistory && (
          <div ref={historyRef} className="history-panel">
            <div style={{ padding: "20px 16px 12px", borderBottom: "1px solid rgba(88,135,255,0.1)" }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.4)", letterSpacing: "0.1em" }}>HISTORY</div>
            </div>
        ...
        {/* Header */}
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "42px 28px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)", position: "relative", zIndex: 10, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => setShowHistory(o => !o)} className={`icon-btn${showHistory ? " active" : ""}`} style={{ width: 30, height: 30, marginRight: 4 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div ref={userMenuRef} style={{ position: "relative" }}>
              <button onClick={() => setShowUserMenu(o => !o)} style={{ width: 30, height: 30, borderRadius: "50%", background: `linear-gradient(135deg,${COLORS.violet},${COLORS.blue})`, border: "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "white" }}>
                {user.email[0].toUpperCase()}
              </button>
              {showUserMenu && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#0d2339", border: "1px solid rgba(88,135,255,0.2)", borderRadius: 10, overflow: "hidden", minWidth: 180, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", animation: "fadeUp 0.12s ease-out", zIndex: 100 }}>
                  <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(88,135,255,0.1)" }}>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</div>
                  </div>
                  <button onClick={signOut} style={{ width: "100%", padding: "10px 14px", background: "transparent", border: "none", color: "rgba(239,68,68,0.7)", fontSize: 12, fontFamily: "'Nunito',sans-serif", fontWeight: 700, textAlign: "left", transition: "background 0.1s" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(239,68,68,0.08)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>Sign out</button>
                </div>
              )}
            </div>
          </div>
          ...
          {showWelcome && (
            <div style={{ animation: "fadeIn 0.4s ease-out", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 24, paddingBottom: 40 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7, justifyContent: "center", maxWidth: 600 }}>
                {STARTER_QUESTIONS.map((q, i) => <MagneticButton key={i} onClick={() => sendMessage(q)} animDelay={`${i * 0.06}s`}>{q}</MagneticButton>)}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start" }}>
                ...
                {msg.role === "assistant" && !msg.streaming && msg.content && !msg.error && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3, paddingLeft: 2 }}>
                    ...
                    {msg.mode && msg.mode !== "normal" && <span style={{ fontSize: 10, fontWeight: 700, color: msg.mode === "quiz" ? COLORS.purple : COLORS.sky, opacity: 0.7 }}>{msg.mode}</span>}
        ...
        <div style={{ padding: "10px 20px 14px", borderTop: "1px solid rgba(255,255,255,0.05)", position: "relative", zIndex: 10, flexShrink: 0 }}>
          <div style={{ maxWidth: 700, margin: "0 auto" }}>
            ...
            <div style={{ textAlign: "center", marginTop: 7, fontSize: 10, color: "rgba(255,255,255,0.13)", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              {chatMode !== "normal" && <span style={{ color: chatMode === "quiz" ? COLORS.purple : COLORS.sky, fontWeight: 700, opacity: 0.8 }}>{chatMode === "quiz" ? "quiz mode" : "guide mode"} ·</span>}
